        ...
        <div class="p-5">
            <div class="text-white w-12/12 md:w-4/12">
                ...
                <div class="flex space-x-2 pb-2 items-center text-gray-500">
                    <p class="text-sm font-bold">{{ getYearFromDate(movies.release_date) }}</p>
                    <p class="text-green-500 font-bold text-xs rounded-full py-1 px-2">
                        {{ movies.vote_average }}</p>
                    <p class="text-sm font-bold">|</p>
                    <p class="text-sm font-bold">{{ movies.runtime }} min</p>
                </div>
                ...
                <div class="flex justify-start space-x-3 text-sm text-white py-5 items-center">
                    ...
                    <ButtonComponent title="Salvar" :icon="TrashIcon" bgColor="bg-transparent"
                        borderColor="border-white" hoverColor="hover:bg-transparent" @click="toggleFavorite" />
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    async created() {
        ...
        try {
            ...
            toast.error("Error ao carregar detalhes do filme ou série!", {
                timeout: 2000
            });
            this.isLoading = false;
        }
